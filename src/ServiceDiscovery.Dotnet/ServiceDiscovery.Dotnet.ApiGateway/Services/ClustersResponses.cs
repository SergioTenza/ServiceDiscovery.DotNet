...
internal static class ClustersResponses
{
	internal static Func<ClusterDto, InMemoryConfigProvider, IResult> InsertCluster = (clusterDto, configProvider) =>
		{
			try
			{
	...
	internal static Func<ClusterDto, InMemoryConfigProvider, IResult> UpdateCluster = (clusterDto, configProvider) =>
		{
			try
			{
				var clusters = configProvider.GetConfig().Clusters.ToList();
